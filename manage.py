...
import sys
import subprocess
from hcli_hak11 import package
...
if sys.argv[-1] == 'dry-run':
    ...
    if branch != "master":
        sys.exit("dry-run from a branch other than master is disallowed.")
    os.system("pip uninstall -y hcli_hak11")
    os.system("rm -rf requirements.txt")
    write_requirements()
    os.system("rm -rf hcli_hak11.egg-info")
...
if sys.argv[-1] == 'publish':
    ...
    os.system("pip install -e .")
    os.system("git tag -a %s -m 'version %s'" % ("hcli_hak11-" + version, "hcli_hak11-" + version))
    os.system("git push")
